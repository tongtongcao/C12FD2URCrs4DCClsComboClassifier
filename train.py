...
def parse_args():
    ...
    parser.add_argument("inputs", type=str, nargs="*", default=["input.csv"],
...
class ClusterDataset(Dataset):
    ...
    Each row of the input CSV is expected to have 14 features:
        - First 4 columns: avgWire values (normalized by 112)
        - Next 4 columns: slope values
        - Next 4 columns: x1, y1, x2, y2 for uRWell crosses (normalized by x, y ranges)\
        - Last 2 columns: superlayer of missing clusters
    ...
    def __init__(self, csv_files):
        ...
        # Split features
        X_avg = df.iloc[:, :4].values.astype(np.float32) / 112.0
        X_slope = df.iloc[:, 4:8].values.astype(np.float32)

        # uRWell features (x1, y1, x2, y2)
        X_urwell = df.iloc[:, 8:12].values.astype(np.float32)
        # Normalize x and y
        X_urwell[:, [0, 2]] = (X_urwell[:, [0, 2]]) / 140.0  # x1, x2
        X_urwell[:, [1, 3]] = X_urwell[:, [1, 3]] / 160.0  # y1, y2

        X_superlayer = df.iloc[:, 12:14].values.astype(np.float32) / 6.0
        self.X = np.concatenate([X_avg, X_slope, X_urwell, X_superlayer], axis=1)
